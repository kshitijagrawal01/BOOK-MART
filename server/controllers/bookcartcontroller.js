...
User.update({"_id":userid},{$set:{"address":add,"landmark":landmark,"mobile":mobile,"state":state}},function(err,result){
	...
	else
	{
           ...
           User.find({"_id":userid},{"cartdetails.bookid":true,"_id":false},function(err,result)
        {
             ...
             User.update({"_id":userid},{$push:{"rentedbooks":{$each:[{"bookid":array[i],"price":price,"returndate":someDate}]}}}
		             	,function(err,result)
                       {
	                      if(err)
	                      {
